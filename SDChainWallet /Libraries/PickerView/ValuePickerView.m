...
//    self.stateStr = defaultStr;
    ...
    if(![defaultStr isEqualToString:@""]){
    NSArray * selectArr = [defaultStr componentsSeparatedByString:@","];
    int index = [selectArr[0] intValue];
    NSArray *yearArr = @[@"2017",@"2018",@"2019"];
    NSArray *stateArr = @[yearArr[index-1],selectArr[1]];
    self.stateStr = [stateArr componentsJoinedByString:@","];

//     //也就和for in有一拼，在这装逼用的
//    [selectArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        int index = [obj intValue];
//        if(index <= self.dataSource.count){
//            self.resultArray[idx] = self.dataSource[idx][index];
//            [self.statePicker selectRow:[selectArr[idx] integerValue] inComponent:idx animated:NO];
//        }
//    }];
        
//        [selectArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//            [self.statePicker selectRow:[selectArr[idx] integerValue] - 1 inComponent:idx animated:NO];
//        }];
//    }
...
//        [self.statePicker selectRow:[selectArr[idx] integerValue] - 1 inComponent:idx animated:NO];
//    }];
//    [self.statePicker selectRow:[selectArr[0] integerValue] - 1 inComponent:0 animated:NO];
    NSLog(@"%@",self.stateStr);
    self.resultArray = [NSMutableArray arrayWithArray:[self.stateStr componentsSeparatedByString:@","]];
}
...
{
//    self.resultArray = [NSMutableArray arrayWithCapacity:self.dataSource.count];

    self.resultArray[component] = self.dataSource[component][row];

     
    
//    self.stateStr = [NSString stringWithFormat:@"%@/%@",self.dataSource[component][row], @(row + 1)];
}
...
- (void)finishBtnClicked:(UIButton *)button
{
    ...
    self.valueDidSelect(self.resultArray);
...
-(NSMutableArray *)resultArray{
    if(!_resultArray){
        _resultArray = [NSMutableArray arrayWithCapacity:self.dataSource.count];
        for(int i = 0 ; i < self.dataSource.count; i++){
            self.resultArray[i] = self.dataSource[i][0];
        }
    }
    return _resultArray;
}
