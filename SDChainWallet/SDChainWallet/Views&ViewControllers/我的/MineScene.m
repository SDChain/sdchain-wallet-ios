    ...
    if(indexPath.section == 0){
        ...
        if(indexPath.section == 1){
            if(indexPath.row == 0){
                ...
                cell.iconImageView.image = [UIImage imageNamed:@"mine_shouxin"];
                NSString *title = NSLocalizedStringFromTable(@"授信", @"guojihua", nil);
                cell.titleLabel.text = title;
            }
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 0){
        ...
        if(indexPath.row == 0){
            ...
            ShouXinScene *scene = [[ShouXinScene alloc] initWithNibName:@"ShouXinScene" bundle:nil];
            scene.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:scene animated:YES];
        }
