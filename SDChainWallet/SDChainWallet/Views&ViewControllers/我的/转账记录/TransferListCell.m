...
//    self.timeLabel.text = [GlobalMethod htcTimeToLocationStr:timeStr];
    NSString *title1;
    if([dict[@"direction"] isEqualToString:@"incoming"]){
      ...
      title1 = NSLocalizedStringFromTable(@"转入金额", @"guojihua", nil);
    }else{
        ...
        title1 = NSLocalizedStringFromTable(@"转出金额", @"guojihua", nil);
    }
