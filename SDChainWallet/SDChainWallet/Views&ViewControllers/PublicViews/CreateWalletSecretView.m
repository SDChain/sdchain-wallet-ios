...
@interface CreateWalletSecretView ()<UITextFieldDelegate>
...
@implementation CreateWalletSecretView
...
-(UILabel *)briefLabel{
    if(!_briefLabel){
        ...
        NSString *title = NSLocalizedStringFromTable(@"请输入钱包密码", @"guojihua", nil);
        _briefLabel.text = title;
...
-(UIButton *)confirmButton{
    if(!_confirmButton){
        ...
        NSString *title = NSLocalizedStringFromTable(@"确认", @"guojihua", nil);
...
-(UIButton *)cancleButton{
    if(!_cancleButton){
        ...
        NSString *title = NSLocalizedStringFromTable(@"取消", @"guojihua", nil);
...
-(void)confirmAction{
    ...
    if(self.CreateWalletSecretInputBlock){
        self.CreateWalletSecretInputBlock(self.pwdTextField.text);
    }else{
        [HTTPRequestManager createWalletWithPassword:self.pwdTextField.text showProgress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            if(self.CreateWalletSecretCurrectBlock){
                self.CreateWalletSecretCurrectBlock(responseObject[@"account"],responseObject[@"secret"], responseObject[@"userAccountId"]);
            };
        } reLogin:^{
            [GlobalMethod loginOutAction];
        } warn:^(NSString *content) {
            if(self.CreateWalletSecretErrorBlock){
                self.CreateWalletSecretErrorBlock(content);
            }
            ...
            if(self.CreateWalletSecretErrorBlock){
                ...
                self.CreateWalletSecretErrorBlock(title);
            }
        }];
    }
