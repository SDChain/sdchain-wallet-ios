...
@interface TradingSellScene ()<UITextFieldDelegate>
...
@property (nonatomic,strong) NSString *sellCurrencyName;
...
@implementation TradingSellScene
...
-(void)setupData{
    ...
    self.sellCurrencyName = @"SDA";
    self.getCurrencyName = @"CNY";
    self.title1.text = @"卖出SDA";
    self.account1.text = @"";
    self.title2.text = @"收入CNY";
    self.account2.text = @"";
    self.amountTextView.textField.text = @"";
    self.priceTextField.textField.text = @"";
    
    [self payQueryCounterpartyWithCurrency:@"SDA" isFirst:YES];
...
#pragma mark - action
//根据资产列表查询收入币种对应
-(void)sellQueryCounterpartyWithCurrency:(NSString *)currency{
...
-(void)payQueryCounterpartyWithCurrency:(NSString *)currency isFirst:(BOOL)first{
    ...
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ %@",restStr,self.sellCurrencyName]];
    ...
    [attrStr addAttribute:NSForegroundColorAttributeName
                    value:NAVIBAR_COLOR
                    range:NSMakeRange(restStr.length, self.sellCurrencyName.length+1)];
    self.restAmountLabel.attributedText = attrStr;
    
    //刷新冻结
    NSMutableAttributedString *attrStr1 = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ %@",[GlobalMethod formatDouble:self.freezeAmount],self.sellCurrencyName]];
    ...
    [attrStr1 addAttribute:NSForegroundColorAttributeName
                     value:NAVIBAR_COLOR
                     range:NSMakeRange([GlobalMethod formatDouble:self.freezeAmount].length, self.sellCurrencyName.length+1)];
...
-(void)countLabelCountAction{
    ...
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ %@",countStr,self.getCurrencyName]];
    [attrStr addAttribute:NSForegroundColorAttributeName
                    value:TEXT_COLOR
                    range:NSMakeRange(0, countStr.length)];
    [attrStr addAttribute:NSForegroundColorAttributeName
                    value:NAVIBAR_COLOR
                    range:NSMakeRange(countStr.length, self.getCurrencyName.length+1)];
    self.countLabel.attributedText = attrStr;
}

-(void)buyButtonAction{
    if([self.sellCurrencyName isEqualToString:self.getCurrencyName]){
        [self presentAlertWithTitle:@"相同币种不能卖出" message:@"" dismissAfterDelay:1.5 completion:nil];
        ...
        [self presentAlertWithTitle:@"卖出单价不能为空" message:@"" dismissAfterDelay:1.5 completion:nil];
    }else if (self.amountTextView.textField.text.length == 0){
        [self presentAlertWithTitle:@"卖出数量不能为空" message:@"" dismissAfterDelay:1.5 completion:nil];
    }else if (self.amount == 0){
        [self presentAlertWithTitle:@"卖出数量不能为0" message:@"" dismissAfterDelay:1.5 completion:nil];
    }else if (self.price == 0){
        [self presentAlertWithTitle:@"卖出数量不能为0" message:@"" dismissAfterDelay:1.5 completion:nil];
    }else if (fmodf(self.totalCount*1000000, 1)!=0){
        [self presentAlertWithTitle:@"收入金额最多精确到小数点后6位" message:@"" dismissAfterDelay:1.5 completion:nil];
    }else{
        //支付密码
        PaymentSecretManager *manager = [PaymentSecretManager shareInstance];
        __weak TradingSellScene *weakSelf = self;
...
-(void)requestBuyActionWithPassword:(NSString *)password{
    ...
    [HTTPRequestManager submitOrderWithWalletPassword:password paysCurrency:self.sellCurrencyName paysCounterparty:self.account1.text paysValue:self.amountTextView.textField.text getsCurrency:self.getCurrencyName getCounterparty:self.account2.text getsValue:[GlobalMethod formatDouble:self.totalCount] type:@"sell" showProgress:YES success:^(NSURLSessionDataTask *task, id responseObject) {
        [self presentAlertWithTitle:@"卖出成功" message:@"" dismissAfterDelay:1.5 completion:nil];
        if(self.TradingSellSuccessBlock){
            self.TradingSellSuccessBlock();
        }
    } reLogin:^{
        [GlobalMethod loginOutAction];
    } warn:^(NSString *content) {
...
-(void)getExchangeListsAction{
    [HTTPRequestManager getExchangeListsWithBaseCurrency:self.sellCurrencyName baseCounterparty:self.account1.text counterCurrency:self.getCurrencyName counterCounterparty:self.account2.text showProgress:YES success:^(NSURLSessionDataTask *task, id responseObject) {
...
-(void)titleButtin1Action{
    ...
    popView.currencyBlock = ^(TradingCurrencyModel *model) {
        weakSelf.title1.text = [NSString stringWithFormat:@"卖出%@",model.currency];
        weakSelf.account1.text = [model.currency isEqualToString:@"SDA"] ? @"" : model.counterparty;
        weakSelf.sellCurrencyName = model.currency;
...
-(void)titleButtin2Action{
    ...
    __weak TradingSellScene *weakSelf = self;
    popView.currencyBlock = ^(TradingCurrencyModel *model) {
        weakSelf.title2.text = [NSString stringWithFormat:@"收入%@",model.currency];
        weakSelf.account2.text = [model.currency isEqualToString:@"SDA"] ? @"" : model.counterparty;
        weakSelf.getCurrencyName = model.currency;
        [weakSelf sellQueryCounterpartyWithCurrency:model.currency];
    };
    [popView showCurrencyListPopView];
...
-(UIButton *)buyButton{
    if(!_buyButton){
        ...
        [_buyButton setTitle:@"卖出" forState:UIControlStateNormal];
